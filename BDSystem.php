...
function fileDevoirByGroupe(){
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            $res["" + $i] = Array(
                "idDevoir" => $row['idDevoir'],
                "groupeDevoir" => $usergroup,
                "categorieDevoir" => $row['categorieDevoir'],
                "nomDevoir" => $row['nomDevoir'],
                "matiereDevoir" => $row['matiereDevoir'],
                "dateDevoir" => $row['dateDevoir']
            );
            $i = $i+1;
        }
        mysqli_free_result($result);
